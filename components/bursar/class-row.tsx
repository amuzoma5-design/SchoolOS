...
import { updateClass, deleteClass } from "@/lib/classes/actions";

export function ClassRow({ cls }: { cls: { id: string; name: string } }) {
  ...
  async function handleSave() {
    setError(null);
    const result = await updateClass({ classId: cls.id, name });
    if (result.error) {
