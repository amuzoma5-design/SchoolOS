            ...
            {invoices.map((inv: any) => (
              <li key={inv.id} className="py-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-medium capitalize ${statusColor[inv.status] ?? "text-ink"}`}>{inv.status}</span>
                </div>
                <p className="text-sm text-ink/60">
                  Due: NGN {Number(inv.amount_due).toLocaleString()}, Paid: NGN {Number(inv.amount_paid).toLocaleString()}
                </p>
                <a href={`/pay/${inv.payment_token}`} target="_blank" className="text-sm text-trust">Payment link &rarr;</a>
                {inv.payments && inv.payments.length > 0 && (
                  <ul className="mt-1 ml-2 border-l-2 border-line pl-2">
                    {inv.payments.map((p: any) => (
                      <li key={p.id} className="text-sm text-ink/60">
                        NGN {Number(p.amount).toLocaleString()} ({p.source}) - <a href={`/api/receipts/${p.id}`} target="_blank" className="text-trust">Download receipt</a>
                      </li>
                    ))}
                  </ul>
                )}
